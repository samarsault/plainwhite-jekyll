...
Чтобы решить его, нужно понять как мне корректно поставить pytest.
...
deps = ; секции `deps` описываются зависимости, которые требуется доставить.
    pytest  ; ставлю последнюю версию pytest
    parameterized  ; поставлю также себе очень удобный пакет для параметризации тестов
commands = pytest  ; запускаем тесты
```
Вот тут возникает очень тонкий момент. Фиксировать актуальную на момент разработки или всегда ставить последнюю?
Если фиксировать, то при установке могут возникнуть конфликты между пакетами из-за различных используемых версий. Если же не фиксировать, то пакет может неожиданно перестать работа. Последняя ситуация очень неприятная для конечных продуктов, когда в одну ночь все билды могут "покраснеть" из-за минорного обновления неявной зависимости. И по [закону Мерфи](https://en.wikipedia.org/wiki/Murphy%27s_law) это произойдет в день релиза.
...
envlist =
    ...
    coverage_report
```
После выполнения в корне проект должна появится папка htmlconv с файлом index.html с красивым отчетом.
...
commands =
  ...
  coverage html
  coverage xml
  codecov -f coverage.xml --token=2455dcfa-f9fc-4b3a-b94d-9765afe87f0f  ; Токен моего проекта в codecov
```
...
tox -e build_wheel
```
В результате получится файл TODO

### Вот теперь заливаем!
Не совсем. 

Для начала стоит проверить, что пакет работает корректным образом. Поэтому сначала заливаю в тестовый репозиторий пакетов.
Использую для этого пакет [twine](https://pypi.org/project/twine/) - инструмент для заливки артефактов в PyPi.
```
[testenv:test_upload]
deps = twine ; ставим последнюю версию
commands =    
...
* Зависимости обновляются и это [следует отслеживать](https://PyUp.io). Как лучше ими управлять?;
