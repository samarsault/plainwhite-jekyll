...
src/
    ...
    tests
        ...
        test_roman.py
```
Тесты в пакет класть не буду, поэтому отделяем ~зёрна от плевел~. Изначально папку src/ не создавал, но дальнейшее развитие показало, что мне так удобнее оперировать. Это не обязательно, поэтому по желанию.

Теперь нужно понять, как писать тесты и запускать их.

Решил воспользоваться популярным фреймворком для тестирования - [pytest](https://docs.pytest.org/en/latest/).
Но я слышал, что ставить `pytest` (как и любые другие зависимости) в системный python - не очень умная идея...
...
Заполняю тело функций и заставляю тесты корректно выполняться.
...
Может быть спорным вариантом выделение все в папку `src/`. Я исходил из следующих соображений:
* Легче указывать только одну папку различным линтерам;
* Не раздувать корень проекта, так как со временем колиество файлов и папок конфигурации в корне только увеличивается;
Но столкнулся с некоторыми неудобствами:
* Потребуется дополнительная настройка PyCharm - указать папку src, как папку исходников.
Возможно, правильнее было бы все таки оставить все в корне.
